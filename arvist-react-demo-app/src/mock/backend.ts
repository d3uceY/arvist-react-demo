/**
 * A scripted Arvist backend, copied from the SDK's own `examples/packstation`
 * reference so this demo can run standalone with no live deployment or
 * credentials. See @arvist-stack/arvist-react-sdk/examples/packstation for
 * the original; nothing here is demo-specific except this comment block.
 *
 ...
 * `{ message, shipment }`, and several writes return a plain JSON string.
 * That is deliberate: a mock that normalises the API would hide the very
 * bugs this example exists to catch.
 *
 * It implements the handful of endpoints the SDK touches and emits the same
 * realtime topics the API does, which makes it a genuine exercise of the
 * SDK's wiring rather than a picture of one.
 */
